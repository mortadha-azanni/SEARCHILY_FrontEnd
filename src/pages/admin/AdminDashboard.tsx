...
export default function AdminDashboard() {
  ...
  return (
    <div className="p-6 md:p-8 flex-1 w-full bg-warm-ivory">
      <div className="mb-8">
        <h1 className="text-3xl font-normal tracking-tight text-mistral-black break-words">Admin Dashboard</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        {metrics.map((metric, i) => (
          <div key={i} className="bg-white p-6 border border-mistral-black/10 rounded-none shadow-sm">
            <h3 className="text-sm font-normal uppercase tracking-wider text-mistral-black/50 mb-2">{metric.title}</h3>
            <div className="flex items-end gap-3">
              <span className="text-3xl font-normal text-mistral-black">{metric.value}</span>
              <span className={`text-sm font-normal mb-1 ${metric.isPositive ? 'text-green-600' : 'text-red-500'}`}>
                {metric.change}
              </span>
            </div>
          </div>
      ...
      {/* Chart Area */}
      <div className="bg-white border border-mistral-black/10 rounded-none p-6 shadow-sm mb-10">
        <h2 id="chart-heading" className="text-lg font-normal tracking-wide text-mistral-black mb-6 uppercase">Search Volume (7 Days)</h2>
      ...
      <div className="bg-white border border-mistral-black/10 rounded-none p-6 shadow-sm">
        <h2 className="text-lg font-normal tracking-wide text-mistral-black mb-6 uppercase">Recent Activity</h2>
        <div className="space-y-4">
          {[1, 2, 3].map((_, i) => (
            <div key={i} className="flex justify-between items-center py-3 border-b border-mistral-black/10 last:border-0 last:pb-0">
              <div>
                <p className="text-sm font-normal text-mistral-black">ETL Sync Completed</p>
